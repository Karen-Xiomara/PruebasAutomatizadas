...
function loginGhost(){
    ...
    const userGhostAdmin = Cypress.config('userGhostAdmin')
    const passwordGhostAdmin = Cypress.config('passwordGhostAdmin')

    cy.visit(loginUrl)
    cy.intercept('https://static.ghost.org/v3.0.0/images/*').as('getConfig')
    cy.get('.email.ember-text-field.gh-input.ember-view').type(userGhostAdmin)
    cy.get('.password.ember-text-field.gh-input.ember-view').type(passwordGhostAdmin)
